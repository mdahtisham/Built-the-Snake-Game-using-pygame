...
def plot_snanke(gameWindowSize, color, snk_list, snk_len):
    for x, y in snk_list:
        pygame.draw.rect(gameWindowSize, color, [x, y, snk_len, snk_len])
...
def gameloop():
    ...
    while not exit_game:
        ...
        # Game Over and game over text on screen 
        if game_over:
            gameWindowSize.fill(white)
            scoreOn_screen("Game Over!!!", Red, Screen_width/2.5, Screen_height/2.5)
            scoreOn_screen("Press ENTER KEY to Restart the Game", Red, Screen_width/7, Screen_height/2)
            ...
            if abs(snk_x - food_x)<10 and abs(snk_y - food_y)<10 :
                ...
                food_x = random.randint(30, 950)        # Screen_widht value is 1000
                food_y = random.randint(30, 550)       # Screen_height value is 600
                snk_SizeIncrease = snk_SizeIncrease + 5
            ...
            # Game Over 
            if snk_x<0 or snk_x>1000 or snk_y<0 or snk_y>600:
                game_over = True
            ...
            plot_snanke(gameWindowSize, Red, snk_list, snk_len)
            ...
            if len (snk_list)>snk_SizeIncrease:
                del snk_list[0]
